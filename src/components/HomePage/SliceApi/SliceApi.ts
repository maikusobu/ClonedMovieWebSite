...
export const getPopularMovie = createAsyncThunk(
  ...
  async ({ page, sort, id, min, max } : {page: number, sort: string, id: number | null, min: number, max: number}, thunkAPI) => {
    const getDB = await fetch(
...
const genreSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(getListGenre.fulfilled, (state, action: PayloadAction<any>) => {
    ...
    builder.addCase(getPopularMovie.fulfilled, (state, action : PayloadAction<any>) => {
...
export const PopularMovies = (state : RootState) => state.genreReducer.dataPopular;
export const PopularStatus = (state : RootState) => state.genreReducer.statusPopular;
